...
class UserfundsController extends Controller
{
    ...
    public function index()
    {
        if (auth()->user()->type == 'admin') {
            $usefunds = User_fund::with('Fund')->get();
        } else {
    ...
    public function review($id, $type)
    {
        ...
        if (auth()->user()->type == 'admin' || $requestreview->emp_id == auth()->user()->id) {
            ...
            store_history(Auth::user()->id, $employee_log);
            return view($this->folderView . 'details', compact('type', 'requestreview', 'empolyers', 'main_banks','banks', 'histories', 'user'));
        } else {
    ...
    public function redirect_bank(Request $request, $id)
    {
        ...
        return redirect()->route('userfunds')->with('success', trans('تم التحويل للبنك'));
    }
    ...
    {
        $file_name = User_fund::with('Files_img', function ($q) {
            $q->select('file_name');
        })->find($id);
